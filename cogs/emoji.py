...
BLOB_GUILD_IDS = [
    ...
    356869031870988309,  # Blob Emoji 2: Google blobs
    356876866952364032,  # Blob Emoji 3: Emotions
    356876897403011072,  # Blob Emoji 4: Actions
    396144014128054275,  # Blob Emoji 5: Characters
    494946907181809667,  # Blob Emoji 6: Google / Animals
    550059457011515417,  # Blob Emoji Inactive Server
]
...
class BlobEmoji(commands.Converter):
    async def convert(self, ctx, argument):
        emojis = dict()
        for g in BLOB_GUILD_IDS:
            guild = ctx.bot.get_guild(g)
            emojis.update({e.id: e for e in guild.emojis})
    ...
    async def convert(cls, ctx, argument):
        try:
            ...
            try:
                ...
                raise commands.BadArgument('Not a valid or supported emoji URL.')
        else:
...
class Emoji(commands.Cog):
    ...
    async def do_redirect(self, message):
        ...
        ch = self.bot.get_channel(792641085506977822)
        if ch is not None:
    ...
    async def on_message(self, message):
        ...
        # Handle the redirection from #emoji-suggestions
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        # we only care if an emoji is added
        ...
        if guild.id not in BLOB_GUILD_IDS:
            return  # not a guild we care about

        # this is the backup channel
        channel = self.bot.get_channel(792646654398300202)
        if channel is None:
            return
    ...
    async def get_all_blob_stats(self, ctx):
        ...
        for g in BLOB_GUILD_IDS:
            ...
            blob_ids.update({e.id: e for e in guild.emojis if len(e.roles) == 0})
    ...
    @checks.is_in_guilds(*BLOB_GUILD_IDS)
    @commands.is_owner()
    async def blobsort(self, ctx):
        """Sorts the blob posts."""
        ...
        for number, page in enumerate(pages, 1):
            ...
            fp.write(b'\n')
        fp.seek(0)
        await ctx.author.send(file=discord.File(fp, 'blob_posts.txt'))
    ...
    async def emojistats_guild(self, ctx):
        ...
        if not emoji_ids:
            return await ctx.send('This guild has no custom emoji.')
    ...
    async def _emoji_create(self, ctx, name: emoji_name, *, emoji: EmojiURL):
        ...
        if emoji_count >= ctx.guild.emoji_limit:
            return await ctx.send('There are no more emoji slots on this server.')
