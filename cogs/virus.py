...
from .utils import formats, storage

GENERAL_ID = 718378271800033321
TESTING_ID = 782350873728647178
MOD_TESTING_ID = 744484453618941962
INFECTED_ROLE_ID = 787672667033567253
EVENT_ID = 787671571598409758
HEALER_ROLE_ID = 787673919389433876
...
class Participant:
    ...
    sickness: int = 0
    backpack: typing.Dict[str, int] = dataclasses.field(default_factory=dict)
    ...
    def base_healing(self):
        ...
        # ergo, the less sick you are, the better your healing powers
    ...
    async def use(self, ctx, item: Item):
        state = await item.use(ctx, self)
        self.backpack[item.emoji] -= 1
        return state

    def heal(self, other: Participant):
        if self.is_dead():
            raise VirusError('<:rooThink:787684353647443998>')
        ...
        if other.healer:
            raise VirusError("I'm sure they know how to treat themselves, we've got others to worry about now.")
    ...
    def hug(self, other: Participant):
...
class Item:
    ...
    def __post_init__(self, data_type: int):
    ...
    async def use(self, ctx, user: Participant):
        return await self._caller(self, ctx, user)

    def usable_by(self, user: Participant):
        return not user.is_dead() and not user.is_cured() and self._pred(self, user)

    def is_buyable_for(self, user: Participant):
...
class Virus(commands.Cog):
    """The virus has spread and needs to be contained \N{FACE SCREAMING IN FEAR}"""
    ...
    def init_storage(self):
        ...
        return {
            'participants': {},
            'stats': Stats(),
            'store': [Item(**data) for data in items.raw],
            'next_cycle': None,
            'event_started': None,
        }

    def cog_check(self, ctx):
        return not self.is_over() and ctx.channel.id in (TESTING_ID, MOD_TESTING_ID, GENERAL_ID)
    ...
    async def new_virus_day(self, guild, infected=None, healers=None, new_infected=5, new_healers=2):
        ...
        infected_ret = set()
        healers_ret = set()
        for channel_id in (GENERAL_ID, TESTING_ID):
            channel = guild.get_channel(channel_id)
            authors = {
                m.author async for m in channel.history(limit=500)
        ...
        # Initialise and fill the storage
        ...
        if infected_mentions:
            await self.log_channel.send(f'{formats.human_join(infected_mentions, final="and")} are suddenly infected.')
        if healer_mentions:
            await self.log_channel.send(f'{formats.human_join(healer_mentions, final="and")} are suddenly healers...?')
    ...
    def log_channel(self):
        return self.bot.get_guild(GUILD_ID).get_channel(EVENT_ID)

    async def infect(self, user: Participant):
    ...
    async def reinfect(self, user: Participant):
    ...
    async def kill(self, user: Participant):
        self.storage['stats'].dead += user.kill()
        await self.storage.save()
        await self.send_dead_message(user)

    async def cure(self, user: Participant):
        user.sickness = 0
        self.storage['stats'].cured += 1
        await self.storage.save()
        await self.send_cured_message(user)

    async def potentially_infect(self, channel_id, participant: Participant):
        ...
        # 0 sickness -> 0% chance, 100 sickness -> 100% chance
        # of getting infected
    ...
    async def surround_healing(self, channel_id, healer: Participant):
        participants = self._authors[channel_id]
        if len(participants) == 0:
            return

        # The surround healing algorithm is based on a few things
        # 1) a base healing rate which is inversely related to the sickness
    ...
    async def apply_sickness_to_all(self, channel: discord.TextChannel, sickness, *, cause=None):
    ...
    async def send_dead_message(self, participant):
        ...
        dialogue = [
            f'A moment of silence for {ping} <:rooBless:787707986499076156> (by the way, {total} dead so far).',
            f"Some people die, some people live, others just disappear altogether. "
            f"One thing is certain: {ping} isn't alive. Oh also, {total} dead so far.",
            f"Got a letter syaing that {ping} isn't with us anymore. "
            f"Can you believe we have {total} dead to this thing?",
            f"Uh... {ping} died? I don't even know who {ping} is lol. Well anyway that's {total} dead.",
            f"Someone that goes by {ping} died. RIP. Kinda forgot what's supposed to go here. "
            f"Oh yeah {total} dead so far.",
    ...
    async def send_infect_message(self, participant):
        ...
        dialogue = [
            ...
            f'"from:{ping} infected" might bring up some interesting results <:rooThink:787684353647443998> '
            f'({total} infected).',
        ]
    ...
    async def send_healer_message(self, participant):
        ...
        try:
            await self.log_channel.send(f'{ping} is now a healer...? Wonder what that means. '
                                        f'Rather rare, only {total} of them.')
        except discord.HTTPException:
            pass
    ...
    async def send_healer_remove_message(self, participant):
        ...
        try:
            await self.log_channel.send(f'{ping} is no longer a healer due to a fatal accident. '
                                        f'Only {total} remain now.')
        except discord.HTTPException:
            pass
    ...
    async def vaccinate(self, user: Participant):
        user.sickness = 0
    ...
    async def on_regular_message(self, message: discord.Message):
        ...
        if user.healer:
            ...
            # which means that they should both do their passive heal and also
            # get increasingly sick
    ...
    async def shop_buy(self, ctx, *, emoji: str):
        ...
        if item is None:
            dialogue = [
                ...
                "Hmm... what are you buying? <:rooThink:787684353647443998>",
    ...
    async def shop_buy_error(self, ctx, error):
        if isinstance(error, commands.CheckFailure) and self._shop_restocking:
            await ctx.send('Sorry, the store is being restocked right now...')
    ...
    async def backpack_use(self, ctx, *, emoji: str):
        ...
        try:
            ...
            dialogue = [
                'Buddy ya think this item exists or you have it? <:notlikeblob:787708035731816449>',
                "I don't know where you've heard of such items <:whenyahomiesaysomewildshit:787708135056867368>",
                "They told me this item doesn't exist bro. Run along now <:blobsweats:787708080900931585>",
        ...
        item = discord.utils.get(self.storage['store'], emoji=emoji)
        if item is None:
            return await ctx.send('Uh... if this happens tell VJ since this is pretty weird.')
    ...
    async def gm_items(self, ctx):
        ...
        store = self.storage['store']
        to_send = [
            f'{i.emoji}: `<in_stock={i.in_stock} unlocked={i.unlocked} total={i.total} uses={i.uses}>`' for i in store
        ]
        await ctx.send('\n'.join(to_send))
    ...
    async def hug(self, ctx, *, member: discord.Member):
        ...
        if ctx.author.id == member.id or member.id == ctx.me.id:
            return await ctx.send('<:rooThink:787684353647443998>')
    ...
    async def research(self, ctx):
        ...
        if item is None:
            return await ctx.send('Tell VJ this happened?')
        ...
        item.unlocked = True
        await self.storage.save()
        await self.log_channel.send(f'\N{CHEERING MEGAPHONE} {ctx.author.mention} seems to have found a cure? '
                                    f'Check the store.')
