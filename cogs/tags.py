...
class Tags(commands.Cog):
    ...
    async def tag(self, ctx, *, name: TagName(lower=True)):
        """Allows you tp tag text for later retrieval.
    ...
    async def create(self, ctx, name: TagName, *, content: commands.clean_content):
        ...
        For the global tags that others can use, consider using the tag box.

        Note that the server moderators can delete your tag.
        """
        ...
        if len(content) > 2000:
            return await ctx.send('Tag content should be a maximum of 2000 characters.')
    ...
    async def alias(self, ctx, new_name: TagName, *, old_name: TagName):
        ...
        tag is deleted, the alias will be deleted as well.
    ...
    async def make(self, ctx):
        """Interactively makes a tag for you
        ...
        # reacquire our connection since we need it for the query
        await ctx.acquire()
        ...
        # release while we wait for the response
        await ctx.release()
        ...
        if len(clean_content) > 2000:
            return await ctx.send('Tag content can be a maximum of 2000 characters.')
    ...
    async def guild_tag_stats(self, ctx):
        # I'm not sure how to do this with a single query
        ...
        if len(records) < 3:
            # Fill in with data to ensure that we have a minimum of 3
            records.extend((None, None, None) for i in range(0, 3 - len(records)))
        ...
        value = '\n'.join(f'{emoji}: {name} ({uses} uses)' if name else f'{emoji}: Nothing!'
                          for (emoji, (name, uses, _, _)) in emojise(records))
        ...
        value = '\n'.join(f'{emoji}: <@{author_id}> ({uses} times)' if author_id else f'{emoji}: No one!'
                          for (emoji, (uses, author_id)) in emojise(records))
        ...
        value = '\n'.join(f'{emoji}: <@{owner_id}> ({count} tags)' if owner_id else f'{emoji}: No one!'
                          for (emoji, (count, owner_id)) in emojise(records))
    ...
    async def remove(self, ctx, *, name: TagName(lower=True)):
        ...
        deletion and has Manage Server permissions, then they can also delete it.
        ...
        if deleted is None:
            await ctx.send("Could not delete that tag. Either it does not exist or you do not have permissions to do so.")
            return
        ...
        # The status returns DELETE <count>, similar to the UPDATE above.
        if status[-1] == '0':
