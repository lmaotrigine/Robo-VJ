...
class Entertainment(commands.Cog):
    ...
    @commands.group(aliases=['anilist'], invoke_without_command=True)
    async def anime(self, ctx, *, search: str):
        ...
        if romaji_title != english_title and len(title) + len(romaji_title) < 254:
        ...
        # Format + episodes
        fields = [('Format', ' '.join(word if word in ('TV', 'OVA', 'ONA') else word.capitalize()
        ...
        fields.extend((('Popularity', media['popularity']), ('Favourites', media['favourites'])))
        ...
        if producers:
            fields.append(('Producers', ', '.join(f'[{producer["name"]}]({producer["siteUrl"]})'
                                                  for producer in producers), len(producers) <= 2))
        # Source
    ...
    async def anime_links(self, ctx, *, search: str):
        ...
        if romaji_title != english_title and len(title) + len(romaji_title) < 254:
            title += f' ({romaji_title})'
        embed = discord.Embed(title=title, url=media['siteUrl'])
        ...
        if isinstance(query, str):
            ...
            for target_section in ('Transcript', 'Explanation', 'Discussion'):
                for result in exact_results + results:
                    ...
                    # Find target section
                    section = discord.utils.find(lambda s: target_section in (s['line'], s['anchor']), sections[page_id])
    ...
    async def process_xkcd(self, ctx, url):
        async with self.bot.session.get(url) as resp:
            ...
            data = await resp.json()
        embed = discord.Embed(title=data['title'], url=f'http://xkcd.com/{data["num"]}',
