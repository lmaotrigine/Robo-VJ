    ...
    async def add_mapping(self, ctx, voice: discord.VoiceChannel, text: discord.TextChannel):
        ...
        async with connection.transaction():
            if await self.client.db.fetchrow("SELECT * FROM music WHERE guild_id = $1 and voice_id = $2", ctx.guild.id, voice.id):
                await self.client.db.execute("UPDATE music SET text_id = $1 WHERE voice_id = $2 AND guild_id = $3", text.id, voice.id, guild_id)
            elif await self.client.db.fetchrow("SELECT * FROM music WHERE guild_id = $1 and text_id = $2", ctx.guild.id, text.id):
                await self.client.db.execute("UPDATE music SET voice_id = $1 WHERE text_id = $2 AND guild_id = $3", voice.id, text.id, guild_id)
            else:
        ...
        await ctx.send(f"Mapped {voice.mention} with {text.mention}. Make sure to set permissions for {text.mentions} accordingly.")
