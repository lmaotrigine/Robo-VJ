...
class Maze:
    ...
    def __init__(self, rows, columns, random_start=False, random_end=False):
        ...
        self.visible[row_offset] = self.visible[row_offset][:column_offset] + 'E' + self.visible[row_offset][
                                                                                    column_offset + 1:]
    ...
    def update_visible(self):
        ...
        for row in range(row_offset, row_offset + 3):
            self.visible[row] = self.visible[row][:column_offset] + self.row_strings[row][
                                                                    column_offset:column_offset + 5] + self.visible[
                                                                                                           row][
                                                                                                       column_offset + 5:]
        row_offset += 1
        column_offset += 2
        self.visible[row_offset] = self.visible[row_offset][:column_offset] + 'I' + self.visible[row_offset][
                                                                                    column_offset + 1:]
    ...
    def move(self, direction):
        ...
        self.visible[row_offset] = self.visible[row_offset][:column_offset] + ' ' + self.visible[row_offset][
                                                                                    column_offset + 1:]
...
class MazeCog(commands.Cog, name="Maze"):
    ...
    async def maze(self, ctx, height: int = 5, width: int = 5, random_start: bool = False, random_end: bool = False):
        ...
        while not reached_end:
            ...
            new_message = await ctx.reply(embed=discord.Embed(description=response).set_footer(text=f"Your current position: {maze.column + 1}, {maze.row + 1}"))
            ctx.bot.loop.create_task(self.attempt_delete(move), name="Maze move message deletion")
            ctx.bot.loop.create_task(self.attempt_delete(message), name="Previous maze message deletion")
            message = new_message
        del self.mazes[ctx.channel.id]

    async def attempt_delete(self, message):
        try:
    ...
    async def file(self, ctx):
        ...
        if maze := self.mazes.get(ctx.channel.id):
            ...
            await ctx.reply(embed=discord.Embed(description=":no_entry: There's no maze game currently going on"))

    @maze.command(name="menu", aliases=['m', "menus", 'r', "reaction", "reactions"])
    async def menu_command(self, ctx, height: int = 5, width: int = 5, random_start: bool = False,
...
class MazeMenu(menus.Menu):
    ...
    async def send_initial_message(self, ctx, channel):
        return await ctx.reply(embed=discord.Embed(description=f'```\n{str(self.maze)}\n```').set_footer(text=f"Your current position: {self.maze.column + 1}, {self.maze.row + 1}"))
