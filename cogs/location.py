...
from .utils.location import wind_degrees_to_direction


class Location(commands.Cog):

    """Commands regarding location."""
    ...
    async def country(self, ctx, *, country: str):
        ...
        if len(languages) == 1 and len(languages[0]) > 22:  # 22: Embed field value limit without offset
            ...
            languages = '\n'.join(l.replace('\n', ' ') for l in languages)
        field_title = self.inflect_engine.plural('Language', len(country_data['languages']))
        ...
        if len(currencies) == 1 and len(currencies[0]) > 22:  # 22: embed field value limit without offset
            ...
            currencies = '\n'.join(c.replace('\n', ' ') for c in currencies)
        field_title = self.inflect_engine.plural("currency", len(country_data["currencies"]))
        ...
        if country_data["regionalBlocs"]:
            ...
            field_title = self.inflect_engine.plural("Regional Bloc", len(country_data["regionalBlocs"]))
            fields.append((field_title, regional_blocs))
        # Borders
        if country_data["borders"]:
            field_title = self.inflect_engine.plural("Border", len(country_data["borders"]))
            fields.append((field_title, '\n'.join(
                ", ".join(country_data["borders"][i:i + 4]) for i in range(0, len(country_data["borders"]), 4))))
        # Timezones
        field_title = self.inflect_engine.plural("Timezone", len(country_data["timezones"]))
        fields.append((field_title, '\n'.join(
            ", ".join(country_data["timezones"][i:i + 2]) for i in range(0, len(country_data["timezones"]), 2))))
        # Demonym
        if country_data["demonym"]:
            fields.append(("Demonym", country_data["demonym"]))
        # Top-Level Domains
        if country_data["topLevelDomain"][0]:
            field_title = self.inflect_engine.plural("Top-Level Domain", len(country_data["topLevelDomain"]))
            fields.append((field_title, ", ".join(country_data["topLevelDomain"])))
        # Calling Codes
        if country_data["callingCodes"] and country_data["callingCodes"][0]:
            field_title = self.inflect_engine.plural("Calling Code", len(country_data["callingCodes"]))
        ...
        embed = discord.Embed(colour=discord.Colour.blurple(), title=country_name)
    ...
    async def weather(self, ctx, *, location: str):
        ...
        fields = [('Conditions', f'{observation.weather.status}'),
                  ...
                  ('Wind', f'{wind_direction} {observation.weather.wind(unit="km_hour")["speed"]:.2f} km/hr\n'
                           f'{wind_direction} {observation.weather.wind(unit="miles_hour")["speed"]:.2f} mi/hr'),
        ...
        embed.set_thumbnail(url=observation.weather.weather_icon_url())
        for name, value in fields:
            embed.add_field(name=name, value=value)
        await ctx.reply(embed=embed)
